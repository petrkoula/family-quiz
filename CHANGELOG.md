...
### Changed
- Migrated the test suite from Taiko E2E (browser automation, dev-server
  dependent) to **Vitest + @testing-library/vue** running in jsdom. Tests are now
  hand-written, editable, and organized around spec contracts with a page-object
  selector layer (`vue-app/tests/support/`). See [TESTING.md](TESTING.md).
- Coverage now spans presenter, landing library, quick-start, and customization
  flows as `vue-app/tests/*.spec.js`.

### Removed
- Taiko, `cross-env`, the `test:e2e*` scripts, and the browser/screenshot CI job.
- Leftover duplicate spec folders from the removed acceptance-codegen pipeline.
